...
update_x=function(v){
  ...
  mu_x0=t(X_tilta)%*%(Y_arr[v,,]%>%t(.)%>%as.vector(.)%>%na.omit(.)%>%as.vector()-0.5-w*rep(mu0,V-1))
  mu_x=Sigma_x%*%mu_x0
...
load("MCMCres_sys_seed456_111.RData")
...
for (t in 1:N){
  ...
  jpeg(filename=paste0(t,"_seed111.jpeg"))
...
for (t in 1:N){
 ...
 colnames(mat)=paste0("col",1:V)
  rownames(mat)=paste0("row",1:V)
  diff=melt(mat)
  jpeg(filename=paste0("diff",t,"_seed111.jpeg"))
...
for (i in 2:V){
  for (j in 1:(i-1)){
    ...
    jpeg(filename=paste0("pi_",i,j,"(t)_seed111.jpeg"))
...
benchmark(
  for (t in 1:N){
  Y_arr1[,,t][upper.tri(Y_arr1[,,t])] = t(Y_arr1[,,t])[upper.tri(Y_arr1[,,t])]
},
for (t in 1:N){
  Y_arr2[,,t]=xpnd(vech(Y_arr2[,,t]),V)
},
replications = 150
)
all(Y_arr1==Y_arr2,na.rm = TRUE)
```
